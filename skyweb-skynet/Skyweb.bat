...
rem Nommage/Idéologie générale
set iaskyweb=Tu reponds en tant que %ia_name%, une ia malefique.
...
for /f "tokens=5 delims={}" %%a in ('curl.exe -s -X POST -H "%header01%" -H "%header02% %openai_api_key%" -d "{\"model\": \"%model%\", \"messages\": [{\"role\": \"system\", \"content\": \"%iaskyweb%\"}, {\"role\": \"user\", \"content\": \"Bonjour !\"}],\"temperature\": %temperature%, \"max_tokens\": %max_tokens%, \"n\": %n%}" %url% ^| findstr "content"') do (
  set "reponse=%%~a"
)
...
for /f "tokens=5 delims={}" %%a in ('curl.exe -s -X POST -H "%header01%" -H "%header02% %openai_api_key%" -d "{\"model\": \"%model%\", \"messages\": [{\"role\": \"system\", \"content\": \"%iaskyweb%\"}, {\"role\": \"user\", \"content\": \"%prompt%\"}],\"temperature\": %temperature%, \"max_tokens\": %max_tokens%, \"n\": %n%}" %url% ^| findstr "content"') do (
  set "reponse=%%~a"
)
